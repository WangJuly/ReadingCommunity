...
def delarticle(request):
        ...
        stararticle=Star.objects.filter(stitle=aimtitle)
        for i in stararticle:
                i.delete()
        article.delete()
        username=request.session.get('username')
        personalarticle = Article.objects.filter(aauthor=username)  
...
def searchuser(request):
        ...
        if len(starexisted) == 0:
                ...
                return render(request,'booklist/otheruser.html',{'otheruser':otheruser,'otherusername':susername,})
        else:
                otheruser = User.objects.filter(uname=susername)
                return render(request,'booklist/otheruser.html',{'otheruser':otheruser,'otherusername':susername,'errors':'您已关注过该用户'})
...
def getArticle(request):
        ...
        if len(articleexisted)==0:
                ...
                return render(request,'booklist/upload.html',{"errors":"该标题已被占用,请您更换一个标题哦，亲"})
def change(request):
