...
def bookarticle(request):
        ...
        return render(request,'booklist/bookarticle.html',{'bookarticle':bookarticle})
def search(request):
    return render(request,'booklist/search.html')
def likeorunlike(request):
        number = Article.objects.filter(number=request.GET["atitle"])
        number.likenumber = number.likenumber+1
